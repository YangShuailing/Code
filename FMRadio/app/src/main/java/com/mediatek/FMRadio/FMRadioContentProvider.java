...
public class FMRadioContentProvider extends ContentProvider {
	...
	private static final String TAG = "FMRadioAPK";
	private static final int STATION_FREQ = 1;
	private static final int STATION_FREQ_ID = 2;

	private static final UriMatcher sURIMatcher = new UriMatcher(UriMatcher.NO_MATCH);
	static {
		sURIMatcher.addURI(FMRadioStation.AUTHORITY, FMRadioStation.STATION, STATION_FREQ);
		sURIMatcher.addURI(FMRadioStation.AUTHORITY, FMRadioStation.STATION + "/#", STATION_FREQ_ID);
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (sURIMatcher.match(uri)) {
			case STATION_FREQ: {
				iRet = sqlDB.delete(TABLE_NAME, selection, selectionArgs);
				getContext().getContentResolver().notifyChange(uri, null);
				break;
			}
			case STATION_FREQ_ID: {
				String stationID = uri.getPathSegments().get(1);
				iRet = sqlDB.delete(TABLE_NAME,
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (v.containsKey(FMRadioStation.Station.COLUMN_STATION_NAME)== false
			...
			long rowId = sqlDB.insert(TABLE_NAME, null, v);
			if (rowId > 0) {
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (sURIMatcher.match(uri)) {
			case STATION_FREQ: {
				iRet = sqlDB.update(TABLE_NAME, values, selection, selectionArgs);
				getContext().getContentResolver().notifyChange(uri, null);
				break;
			}
			case STATION_FREQ_ID: {
				String stationID = uri.getPathSegments().get(1);
				iRet = sqlDB.update(TABLE_NAME,
						values,
						FMRadioStation.Station._ID
						+ "="
						+ stationID

...
public class FMRadioStation {
	...
	public static int FIXED_STATION_FREQ = 1000; // 1000 * 100k Hz
	public static final int HIGHEST_STATION = 1080;
	public static final int LOWEST_STATION = 875;
	public static String CURRENT_STATION_NAME = "FmDfltSttnNm";
	...
	public static void updateStationToDB(Activity activity, String stationName, int stationFreq, int stationType) {
	...
	public static boolean isFavoriteStation(Activity activity, int iStation) {
		/*boolean bRet = false;
		Uri uri = Station.CONTENT_URI;
		Cursor cur = activity.managedQuery(uri, columns, null, null, null);
		if (null != cur) {
			cur.moveToFirst();
			while (!cur.isAfterLast()) {
				if (cur.getInt(cur.getColumnIndex(Station.COLUMN_STATION_TYPE)) == STATION_TYPE_FAVORITE
					&& cur.getInt(cur.getColumnIndex(Station.COLUMN_STATION_FREQ)) == iStation) {
					bRet = true;
					break;
				}
